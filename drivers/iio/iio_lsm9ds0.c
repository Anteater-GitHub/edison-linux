  ...
  case IIO_CHAN_INFO_RAW:
    ...
    case IIO_ANGL_VEL:
      err = lsm9ds0_read_measurements(data->client, 
          LSM9DS0_OUT_X_L_G_REG, &y, &x, &z);
		z = -z;
      scale = 12217;//70000/180*3.1415926 *10 in 0.1 urad/s
      break;
    case IIO_ACCEL:
      err = lsm9ds0_read_measurements(data->client, 
          LSM9DS0_OUT_X_L_A_REG, &y, &x, &z);
			y = -y;
			x = -x;
		  scale = 244*98; // 0.1 ug
      break;
    case IIO_MAGN:
      err = lsm9ds0_read_measurements(data->client, 
          LSM9DS0_OUT_X_L_M_REG, &z, &x, &y);
		  x = -x;
		  y = -y;
      scale = 80*10;// 80ugauss lsb * 10
      break; 
    default:
      return -EINVAL;
    }
...
static int lsm9ds0_gyro_init(struct i2c_client *client)
{
  ...
  data->gyro_scale = 1;// in 1e-6 rad/s   //LSM9DS0_GYRO_FS_2000DPS_GAIN;
...
static int lsm9ds0_accel_magn_init(struct i2c_client *client)
{
  ...
  ret = i2c_smbus_write_byte_data(client, LSM9DS0_CTRL_REG1_XM_REG, 
      LSM9DS0_ACCEL_ODR_200HZ_VAL | LSM9DS0_ACCEL_X_EN | 
  ...
  data->accel_scale = 1; // 1e-6 m/s2
  data->magn_scale = 10; // 1e-6 gauss
...
static int lsm9ds0_remove(struct i2c_client *client)
{
  ...
  iio_device_free(indio_dev);
  dev_info(&client->dev, "LSM9DS0 sensor driver removed.");
  return 0;
}
