...
static void tng_ssp_spi_platform_pinmux(void)
{
	...
	gpio_free(tng_ssp_spi2_CS0_gpio);
...
static int mcp2515_board_specific_setup(struct spi_device *spi) {
	...
	int saved_muxing;
        ...
        dev_dbg(&spi->dev, "%s IRQ for GPIO 49: %d\n", __func__, gpio_to_irq(49));
	...
	err = devm_gpio_request_one(&spi->dev, tng_ssp_spi2_CS0_gpio,
			GPIOF_DIR_OUT|GPIOF_INIT_HIGH, "MCP2515 CS");
	if (err) {
		pr_err("%s: unable to get Chip Select GPIO, fallback to legacy CS mode \n", __func__);
		lnw_gpio_set_alt(tng_ssp_spi2_CS0_gpio, saved_muxing);
		chip.cs_control = NULL;
		chip.platform_pinmux = NULL;
	}

	// The MCP2515 CAN_INT on the anemobox is connected to GP49.
	/*if ((gpio_request(15, "MCP251x CAN INT") == 0) &&
	(gpio_direction_input(15) == 0)) {
		gpio_export(49, 0);
		set_irq_type(gpio_to_irq(49), IRQ_TYPE_EDGE_FALLING);
	} else {
		printk(KERN_ERR "could not obtain gpio for MCP251x CAN bus interrupt\n");
		return;
	}*/
	spi->irq = gpio_to_irq(15);
	return 0;
}
